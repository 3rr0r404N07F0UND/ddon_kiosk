...
$(".numberPad0").click(function () {
  ...
  if ($(".phoneNumberInput0 input").val().length >= maxLength) {
    ...
    $(".phoneNumberInput0 input").val($(".phoneNumberInput0 input").val() + number);
  }
}); //비밀번호 입력 모달 - 지우기(화살표)를 눌렀을 때 지워짐

$("#removeBtn0").click(function () {
  $(".phoneNumberInput0 input").val($(".phoneNumberInput0 input").val().slice(0, -1));
...
$("#inputPW").on("input", function () {
  $("#inputPW").val($("#inputPW").val().replace(/[^0-9]/, ""));
});