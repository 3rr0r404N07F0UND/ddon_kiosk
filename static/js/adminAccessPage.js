$("#submitButton").on("click", () => {
  $("#selectDateStart").val() &&
    $("#selectDateEnd").val() &&
    $("#selectDateStart").val() <= $("#selectDateEnd").val() &&
    $("#submitHidden").trigger("click");
});
$(".orderHistory").each((i, v) => {
  //console.log(v.innerHTML);
  const arrayHtml = JSON.parse(v.innerHTML);
  const numberToArray = [];
  // const arrayHtml2 = [];
  ...
  numberToArray.forEach((v1, i1) => {
    lastOutputArray.push(`${v1} ${outputOrderCount[i1]}개`);
  });
  v.innerHTML = lastOutputArray;
});
$(".totalPrice").each((i, v) => {
  let rememberNumber = v.innerHTML;
