...
$("#submitButton").on("click", function () {
  $("#selectDateStart").val() && $("#selectDateEnd").val() && $("#selectDateStart").val() <= $("#selectDateEnd").val() && $("#submitHidden").trigger("click");
});
$(".orderHistory").each(function (i, v) {
  ...
  numberToArray.forEach(function (v1, i1) {
    lastOutputArray.push("".concat(v1, " ").concat(outputOrderCount[i1], "\uAC1C"));
  });
  v.innerHTML = lastOutputArray;
});
$(".totalPrice").each(function (i, v) {
  var rememberNumber = v.innerHTML;
  console.log(rememberNumber);
  rememberNumber = rememberNumber.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  v.innerHTML = rememberNumber;
});